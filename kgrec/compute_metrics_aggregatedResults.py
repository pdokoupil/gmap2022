...
all_constants_without_u_norm = np.linspace(-0.75, 0.75, 7) #np.linspace(-3.0,3.0,7)

all_group_types = ["sim", "div"]
...
if __name__ == "__main__":
    ...
    assert (not args.rating_normalization) or args.rating_normalization in globals(), f"Unknown rating normalization was given: {args.rating_normalization}"
    assert (args.user_rating_normalization == "identity") or args.user_rating_normalization in globals(), f"Unknown user-rating normalization was given: {args.user_rating_normalization}"
        ...
        for group_size in group_sizes: #["2","3","4","8"]:
            if args.use_all_constants:
                ...
                for c in all_constants:
                    ...
                    results = main(os.path.join(args.path_prefix, "data/"), group_type, group_size, args)
                    f.write(results)
            else:
